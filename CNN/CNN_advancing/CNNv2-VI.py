...
with tf.name_scope('CNN_Hyper_Parameter'):
    ...
    learning_rate = \
        tf.train.exponential_decay(learning_rate=0.03, global_step=global_step, decay_steps=100, decay_rate=0.9,
                                   staircase=True)
...
def cnn(X, SinaX, QiyiX, W_fc, W_c):
    with tf.name_scope('CNN_Convolution_Layer'):
        ...
        # 5 -> 3 (3)池化
        # (8, 1, 1, 19)
        s_u = tf.unstack(W_c['s_c'], axis=3)

        q_cu = tf.unstack(QiyiX, axis=1)

        q_u = tf.unstack(W_c['q_c'], axis=3)
    ...
    for i in range(SinaX.shape[1]):
        ...
        cI = tf.nn.conv1d(tf.cast(Sina_perX, tf.float32), s_u[i], stride=1, padding="VALID",
                          data_format="NHWC")
