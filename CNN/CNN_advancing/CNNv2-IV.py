...
Modeldir = Rootdir + r"\Models\CNNv2-IV\CNNv2.model"
...
with tf.name_scope('CNN_Training_Parameter'):
    ...
    with tf.name_scope('weights'):
        W_fc = {
            ...
            'w2': tf.Variable(tf.random_normal([layer1_size, 1]), name='weights2'),
            'w3': tf.Variable(tf.random_normal([layer2_size, 1]), name='weights3'),
    ...
    with tf.name_scope('biases'):
        ...
        W_c = {
            # sina
            's_c1': tf.get_variable('w1_sina', [5, 1, 1, 19]),
            's_c1b': tf.get_variable('b1_sina', [1]),
            's_c2': tf.get_variable('w2_sina', [5, 1, 1, 19]),
            's_c2b': tf.get_variable('b2_sina', [1]),
            's_c3': tf.get_variable('w3_sina', [5, 1, 1, 19]),
            's_c3b': tf.get_variable('b3_sina', [1]),
            # qiyi
            'q_c1': tf.get_variable('w1_qiyi', [5, 1, 1, 19]),
            'q_c1b': tf.get_variable('b1_qiyi', [1]),
            'q_c2': tf.get_variable('w2_qiyi', [5, 1, 1, 19]),

            'q_c2b': tf.get_variable('b2_qiyi', [1]),
            'q_c3': tf.get_variable('w3_qiyi', [5, 1, 1, 19]),
...
def cnn(X, SinaX, QiyiX, W_fc, W_c):
    with tf.name_scope('CNN_Convolution_Layer'):
        ...
        s_c3u = tf.unstack(W_c['s_c3'], axis=3)
        # (6, 1, 1)
        # (1,)
        ...
        q_c3u = tf.unstack(W_c['q_c3'], axis=3)
    ...
    for i in range(SinaX.shape[1]):
        ...
        cIII = tf.nn.conv1d(cIII, s_c3u[i], stride=1, padding="VALID", data_format="NHWC")
    ...
    for i in range(QiyiX.shape[1]):
        ...
        pII = tf.nn.max_pool(cII, ksize=[1, 5, 1, 1], strides=[1, 1, 1, 1], padding="VALID")
        cIII = tf.squeeze(pII, axis=-1)
        cIII = tf.nn.conv1d(cIII, q_c3u[i], stride=1, padding="VALID", data_format="NHWC")
    ...
    y_ = tf.nn.tanh(tf.matmul(a, W_fc['w2']) + W_fc['b2'])
